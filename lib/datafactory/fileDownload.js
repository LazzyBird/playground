...
export const brokenLinkCounter = async (page, loc) => {
    ...
    for (const link of links.links) {
        ...
        if (response.ok) {
            ...
            const corruptedFile = { "response status text": `${response.statusText}`, "response status code": `${response.status}`, date: `${new Date({dateStyle: "short", timeStyle: "short"})}`, URL: `${url}` };
